...
transfer kernel (v11)}, pdfauthor={Anthony Monnerot}}
...
significantly more demanding in terms of loop-phase stability. Beyond the protocol, we characterize the kernel's intrinsic analyticity boundary at $|x| = 1$ --- emergent from the mode ladder, with a chiral phase law of its branch-cut discontinuity separating the one-way kernel from its real-offset (scalar) control --- and prove an elementary Fourier floor theorem for the interference relief along the cut: the residual relief of large kernel sums is the window-contrast of the Fourier transform of the weighted winding measure, with closed-form floors for the canonical ladder families and an amplitude dichotomy (the relief of a finite ladder is Bohr almost periodic and cannot decay; the amplitude of an absolutely continuous continuum dies away --- the persistence of the interference minima being a separate property, exemplified by the uniform sinc whose exact zeros recur forever). Read backwards, the law yields a spectroscopy of the surviving transfer element, recovering its winding measure from the relief alone, with proven limits. No experimental
...
minima --- and computes the canonical floors in closed form.
...
\begin{equation}
G_{21}(\mathbf{r}_1;\mathbf{r}_2) \;=\; \sum_{m \geq 1} g_m\, x^m,
\qquad x = \zbone\,\wbtwo,
\label{eq:kernelsum}
\end{equation}
...
\item[(c)] the physical Jordan ladder $g_m = k / \lambda_m^2$ with
  ...
  \begin{equation}
  G_{21} \;=\; -\frac{k}{\delta^{2}} \left[\, \Phi(x, 2, c) - \frac{1}{c^{2}} \,\right],
...
\paragraph{Version note (v11).} This version repairs the reciprocity statement
...
direct measurement of $R_{\mathbf{G}}$ requires. The repair was prompted by an external adversarial audit and
is certified by an expanded repair harness built in the same exact symbolic
discipline as the original claims. Intermediate drafts v4--v10 existed only
...
\hline
reciprocal--Hermitian & $g(z,\zb) + g(w,\wb)$, real coefficient, $M(g) = g$ & $R = 0$, $\Delta = 0$ \\
dissipative reciprocal & $c\,(g(z,\zb) + g(w,\wb))$, $c \notin \mathbb{R}$, $M(g) = g$ & $R = 0$, $\Delta \neq 0$ \\
Hermitian decoy & Bergman $-k\log(1 - z\wb)$, $k \in \mathbb{R}$ & $R \neq 0$, $\Delta = 0$ \\
...
$\mathbf{G} = L^{-1}$, i.e.\ to $R_{\mathbf{G},C} = 0$ (reducing to
...
$\partial_a \partial_b \log G_{21}$ across the two points, evaluated on all four
...
$\mathrm{disc}_{\text{one-way}}/\mathrm{disc}_{\text{real-offset}}
...
certification target rather than assert here; physically, the two sector jumps
correspond to the two calibrated transfer configurations ($G_{21}$ and
$G_{12}$). The complex difference is essential: a size
asymmetry ($F_a(X) + \rho F_a(Y)$, $\rho > 0$ real, $\rho \neq 1$; a
unimodular complex $\rho$ would instead shift the angle component of the
weight, not its modulus) leaves phase and slope identical in both sectors and is read only by
the modulus of $W$, with the
exact pointwise law $W(x) = (1-\rho)\,D(x)$ --- hence
$\max_x|W| = |1-\rho|\,\max_x|D(x)|$. All clauses (refusal on the reciprocal
...
summed kernels. A \emph{common} positive rescaling of all the weights is a
...
not a volume change at all). Relative complex phases between the weights move the pattern
itself: for two needles a relative phase $\Delta\varphi = \varphi_2 -
...
$u_n = ((2n+1)\pi - \Delta\varphi)/\Delta\nu$, Section~\ref{sec:calendar}), and for three or more needles it
...
proof is deposited in the repository accompanying this record), establishes
the amplitude dichotomy of the continuum limit for the log-scale calendar
of Section~\ref{sec:calendar}, and computes the floors of the canonical ladder families
...
$\mathcal{C}(f) = (\max_I f - \min_I f)/(\max_I f + \min_I f)$ satisfies
$\mathcal{C}(|\hat{\mu}_N|) \to \mathcal{C}(|\hat{\mu}|)$, the denominator being protected:
...
uniform convergence on the compact window; $\mathcal{C}$ is continuous in the sup
...
splits on the nature of the measure. If $\mu$ is \emph{finitely supported} --- any finite choir ---
...
lemma gives $\hat{\mu}(u) \to 0$, with no universal rate in general;
...
reference beat, read at two reference phases in quadrature, recovers the
...
families, including size-disparate and size--speed-correlated choirs. We state this as an
outlook on the mathematics of the kernel, not as an experimental claim.

\section{Relation to prior structures}
\label{sec:prior}

None of the ingredients used above is new in isolation; what we did not find a direct
precedent for in our search is the combination of the forcing question, the mirror-deletion
...
upstream). The secular envelope itself is therefore not claimed as new; what appears
...
complex winding. We therefore read the kernel as a
...
calibration fixed once in the port basis, $C = I$): $R_{\mathbf{G}}$ is complex-valued, and
modulus-only records are mirror-blind (Section~\ref{sec:limits}). The
...
(energy floor $10^{-\mathrm{isolation}/10}$); within the loop model the test
...
phase-referenced acquisition of (iii). Figures~\ref{fig:envelope}
...
(Section~\ref{sec:dsi}); what we read as apparently new --- the
